...
```
Interesting.  The mean daily total number of steps is `r sprintf("%.2f", mean(dailyTotals$total, na.rm = TRUE))` and the median daily total number of steps is `r median(dailyTotals$total, na.rm = TRUE)`.  We've been removing NA values though and from the histogram it appears that this might be distorting the data so we'll need to come back to this.
...
The mean changed by quite a bit (`r sprintf("%.2f", compare[2,2]-compare[1,2])` steps). The median only changed by `r sprintf("%.2f", compare[2,3]-compare[1,3])` steps.  This isn't surprising as we already know that median is a more robust measure of central tendency and is less affected by extreme values (like forgetting to collect data on 8 days!).
